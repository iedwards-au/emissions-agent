...
<style>
  ...
  html, body { background-color: #0C0E12 !important; }
  .stApp { background-color: #0C0E12 !important; }
  .stAppViewContainer { background-color: #0C0E12 !important; }
  .block-container { background-color: #0C0E12 !important; padding-top: 2rem; max-width: 900px; }
  section[data-testid="stSidebar"] { background-color: #0C0E12 !important; }
  div[data-testid="stAppViewContainer"] { background-color: #0C0E12 !important; }
  div[data-testid="stVerticalBlock"] { background-color: transparent !important; }

  /* Global text */
  html, body, p, div, span, label { color: #FAFFF0 !important; font-family: 'IBM Plex Sans', sans-serif; }

  /* Hide Streamlit chrome */
  #MainMenu, footer, header { visibility: hidden; }

  /* Logo / header */
  .s3-header { display: flex; align-items: center; gap: 12px; margin-bottom: 0.25rem; }
  .s3-logo {
    font-family: 'IBM Plex Mono', monospace;
    font-size: 1.05rem; font-weight: 600;
    color: #DBFC01; letter-spacing: 0.08em; text-transform: uppercase;
  }
  .s3-tagline { font-size: 0.78rem; color: #888 !important; font-weight: 300; }
  h1 {
    font-family: 'IBM Plex Sans', sans-serif;
    font-size: 2rem !important; font-weight: 600 !important;
    color: #FAFFF0 !important; margin-top: 0.25rem !important; line-height: 1.2 !important;
  }
  .accent { color: #DBFC01; }

  /* Divider */
  .s3-divider {
    border: none; border-top: 1px solid #DBFC01;
  ...
  .stRadio label {
    background: #1A1D22 !important; border: 1px solid #2E3138 !important;
    border-radius: 6px !important; padding: 8px 20px !important;
    cursor: pointer !important; font-size: 0.88rem !important;
    color: #FAFFF0 !important; transition: all 0.15s !important;
  }
  .stRadio label:hover { border-color: #DBFC01 !important; color: #DBFC01 !important; }

  /* Text areas + inputs — light background so they're clearly visible */
  ...
  .stTextArea textarea:focus, .stTextInput input:focus {
    ...
    box-shadow: 0 0 0 2px rgba(219,252,1,0.3) !important;
  }
  /* Input labels */
  .stTextArea label, .stTextInput label {
    color: #FAFFF0 !important; font-size: 0.85rem !important;
  }

  /* Primary button */
  .stButton > button {
    background: #DBFC01 !important; color: #0C0E12 !important;
    font-family: 'IBM Plex Mono', monospace !important;
  ...
  .stDownloadButton > button {
    background: transparent !important; color: #DBFC01 !important;
    border: 1px solid #DBFC01 !important;
    font-family: 'IBM Plex Mono', monospace !important;
  ...
  /* Tool call log */
  .tool-log {
    background: #111318; border-left: 2px solid #DBFC01;
    border-radius: 0 6px 6px 0; padding: 10px 14px; margin: 4px 0;
    font-family: 'IBM Plex Mono', monospace; font-size: 0.78rem; color: #82E500;
  }

  /* Response box */
  .response-box {
    background: #13161B; border: 1px solid #2E3138; border-radius: 8px;
    padding: 20px 24px; font-size: 0.9rem; line-height: 1.65;
    color: #FAFFF0 !important; margin-top: 1rem;
  }

  /* Selectbox */
  .stSelectbox > div > div {
    background: #F5F5F0 !important; border-color: #DBFC01 !important; color: #0C0E12 !important;
  }

  /* Expander */
  .streamlit-expanderHeader {
    background: #1A1D22 !important; border-radius: 6px !important;
...
def _try_generate_pdf(result, tool_calls_log, is_analyse):
    try:
        ...
        st.session_state.last_pdf_name = "scope3_report.pdf"
...
if run_clicked:
    ...
    if not message:
        ...
        with st.spinner("Agent thinking..."):
            try:
                ...
                def traced_execute(tool_name, tool_input):
                    with tool_container:
                        st.markdown(
                            f'<div class="tool-log">⚙ Calling <b>{tool_name}</b>…</div>',
                            unsafe_allow_html=True
                        )
            ...
            try:
                tools.execute_tool = _orig_execute
                agent_module.execute_tool = _orig_execute
            except Exception:
                pass
        ...
        _try_generate_pdf(result, result.get("tool_calls", []), is_analyse)
